...
openGlCode=$(curl -X POST https://www.shadertoy.com/shadertoy \
    ...
    -H 'TE: trailers' \
    -d 's=%7B%20%22shaders%22%20%3A%20%5B%22wtVyWK%22%5D%20%7D&nt=1&nl=1&np=1' 2>/dev/null | zcat |jq '.[].renderpass[].code'| sed 's/\\n/\n/g')

nom_de_fichier

trim "$openGlCode" > ${DOSSIER_COURANT}/openGl/
